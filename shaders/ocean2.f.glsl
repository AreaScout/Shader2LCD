...
void main()
{
    ...
    // Camera settings
	vec3 camPos = vec3(0, kCameraHeight, -kCameraSpeed * time);
    vec3 camDir = vec3(kCameraShakeAmp * (rand(vec2(time, 0.0)) - 0.5), kCameraShakeAmp * (rand(vec2(time, 0.1)) - 0.5), -1);
    
    vec3 up = vec3(kCameraRollNoiseAmp * (SmoothNoise(vec3(0.2 * time, 0.0, 0.0)) - 0.5), 1.0, 0.0);
